...
class WC_Aelia_CurrencySwitcher implements IWC_Aelia_CurrencySwitcher {
	...
	protected function user_is_paying_existing_order() {
		...
		// Since WC 2.1 they changed the ID of the payment page (for whatever reason)
		if(version_compare($this->wc()->version, '2.1', '>=')) {
			$payment_page_id = woocommerce_get_page_id('checkout');
		}
		else {
		...
		if(($paying_for_order != false) || ($current_page_id == $payment_page_id)) {
			...
			if(version_compare($this->wc()->version, '2.1', '>=')) {
				...
				$order_id = get_value('order-pay', $wp->query_vars);
			}
			else {
	 ...
	 * settings.
	 *
	 * @param string formatted_order_total The original HTML with the order total.
	 * price and sale price.
	 * @param WC_Order order The order whose is being re-formatted.
	 * @return string The order total, formatted as per Currency Switcher settings.
	 */
	public function woocommerce_get_formatted_order_total($formatted_order_total, $order) {
		if($this->set_active_currency_to_order_currency($order->id)) {
			$formatted_order_total = wc_price($order->order_total, array('currency' => $this->selected_currency));
		}
	...
	public function pre_option_woocommerce_price_num_decimals($decimals) {
		return $this->price_decimals($this->get_selected_currency());
	}
	...
	protected function set_shipping_methods_hooks() {
		if(version_compare($this->wc()->version, '2.1', '>=')) {
			...
			add_filter('woocommerce_available_shipping_methods', array($this, 'woocommerce_package_rates'));
		}
	...
	protected function set_order_hooks() {
		...
		add_filter('woocommerce_get_formatted_order_total', array($this, 'woocommerce_get_formatted_order_total'), 10, 2);
	...
	public function get_selected_currency() {
		...
		if(!self::settings()->debug_geolocation_currency_detection()) {
			...
			$this->selected_currency = coalesce(Aelia_SessionManager::get_value(AELIA_CS_USER_CURRENCY),
																					$user_currency);
		}
	...
	public function woocommerce_shipping_methods($shipping_methods_to_load) {
		...
		foreach($shipping_methods_to_load as $method) {
			...
			if(empty($method->shipping_prices_in_currency)) {
				if(!empty($method->min_amount)) {
					$method->min_amount = $this->convert($method->min_amount,
																							 $this->settings_controller()->base_currency(),
																							 $this->get_selected_currency());
				}
			}
	...
	public function woocommerce_integrations_override($integrations) {
		...
		if(class_exists('WC_Google_Analytics') &&
			 version_compare($this->wc()->version, '2.1', '<')) {
	...
	public function woocommerce_loaded() {
		...
		// Patch for WC2.3 - The initialisation of the hooks for shipping methods had
		// to be moved here, because they have to be processed differently, depending
		// on the WooCommerce version. To get the WC version, we need to wait until
		// the "woocommerce_loaded" event
		...
		if(empty($selected_currency) &&
			 self::settings()->get(WC_Aelia_CurrencySwitcher_Settings::FIELD_CURRENCY_VIA_URL_ENABLED)) {
			$selected_currency = get_value(AELIA_CS_ARG_CURRENCY, $_GET);
		}
