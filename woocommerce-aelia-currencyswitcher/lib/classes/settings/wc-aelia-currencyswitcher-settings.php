...
class WC_Aelia_CurrencySwitcher_Settings extends WC_Aelia_Settings {
	...
	protected function register_exchange_rates_models() {
		...
		$this->register_exchange_rates_model('WC_Aelia_YahooFinanceModel', __('Yahoo! Finance', $this->textdomain));
		$this->register_exchange_rates_model('WC_Aelia_TCBModel', __('Turkey Central Bank', $this->textdomain));
	}
	...
	public function debug_geolocation_currency_detection() {
		return !empty($_GET[AELIA_CS_ARG_DEBUG_GEOLOCATION_DETECTION]) &&
					 $this->debug_mode() &&
	...
	public function price_decimals($currency = null) {
		...
		if(get_value($currency, $this->_price_decimals, null) == null) {
			$this->_price_decimals[$currency] = $this->get_currency_decimals($currency);
		}
		return $this->_price_decimals[$currency];
	}
	...
	public function get_currency_symbol_position($currency) {
		$default_position = $this->woocommerce_currency_position;
		...
		$position = get_value('symbol_position', $currency_settings, $default_position);
		return $position;
	}
	...
	public function update_exchange_rates(array &$settings, &$errors = array()) {
		...
		if(($latest_exchange_rates === null) ||
			...
			$exchange_rates = $settings[self::FIELD_EXCHANGE_RATES];
	...
	public function validate_settings($settings) {
		...
		$processed_settings[self::FIELD_CURRENCY_VIA_URL_ENABLED] = get_value(self::FIELD_CURRENCY_VIA_URL_ENABLED, $settings, 0);
