...
class WC_Aelia_CurrencySwitcher_Settings_Renderer extends WC_Aelia_Settings_Renderer {
	...
	protected function add_settings_fields() {
		...
		// Prepare select to enable forcing the currency by billing country
		add_settings_field(
			$currency_by_billing_country_enabled_field_id,
			'<span id="currency_by_billing_country">' .
	    __('Force currency selection by billing country', $this->_textdomain) .
			'</span>' .
			'<p>' .
			__('When enabled, it forces the shop currency to the one in use in the billing country '.
		...
		$currency_via_url_enabled_field_id = WC_Aelia_CurrencySwitcher_Settings::FIELD_CURRENCY_VIA_URL_ENABLED;
		$currency_via_url_enabled = (int)$this->current_settings($currency_via_url_enabled_field_id);

		$shop_url = $shop_page_url = get_permalink(woocommerce_get_page_id('shop'));
		...
		$shop_url .= AELIA_CS_ARG_CURRENCY . '=USD';
		$shop_url = "<a href=\"$shop_url\">$shop_url</a>";

		// Prepare select to allow choosing how often to update the Exchange Rates
		add_settings_field(
			$currency_via_url_enabled_field_id,
			'<span id="currency_via_url">' .
	    ...
	    array(
				'settings_key' => $this->_settings_key,
				'id' => $currency_via_url_enabled_field_id,
				'label_for' => $currency_via_url_enabled_field_id,
				// Input field attributes
				'attributes' => array(
					'class' => $currency_via_url_enabled_field_id,
					'checked' => $currency_via_url_enabled,
				),
			)
		);
		...
		if(function_exists('wc_get_log_file_path')) {
			$log_file_path = wc_get_log_file_path($log_id);
		}
		else {
			$log_file_path = sprintf('woocommerce/logs/%s-%s.txt', $log_id, sanitize_file_name(wp_hash($log_id)));
		}
